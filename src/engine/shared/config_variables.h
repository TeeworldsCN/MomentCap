...
MACRO_CONFIG_INT(SvSnapshotInterval, sv_snapshot_interval, 0, 0, 10, CFGFLAG_SERVER, "Use high bandwidth mode. Doubles the bandwidth required for the server. LAN use only")
...
MACRO_CONFIG_INT(SvSaveGames, sv_savegames, 0, 0, 0, CFGFLAG_SERVER, "Enables savegames (/save and /load)")
MACRO_CONFIG_INT(SvSaveGamesDelay, sv_savegames_delay, 60, 0, 10000, CFGFLAG_SERVER, "Delay in seconds for loading a savegame")
MACRO_CONFIG_INT(SvUseSQL, sv_use_sql, 0, 0, 0, CFGFLAG_SERVER, "Enables MySQL backend instead of SQLite backend (sv_sqlite_file is still used as fallback write server when no MySQL server is reachable)")
...
MACRO_CONFIG_INT(SvTeam, sv_team, 1, 0, 1, CFGFLAG_SERVER | CFGFLAG_GAME, "Teams configuration (0 = off, 1 = on but optional, 2 = must play only with teams, 3 = forced random team only for you)")
MACRO_CONFIG_INT(SvTeamMaxSize, sv_max_team_size, 4, 1, MAX_CLIENTS, CFGFLAG_SERVER | CFGFLAG_GAME, "Maximum team size (from 2 to 64)")
MACRO_CONFIG_INT(SvMapVote, sv_map_vote, 0, 0, 0, CFGFLAG_SERVER, "Whether to allow /map")
...
MACRO_CONFIG_INT(ConnTimeout, conn_timeout, 15, 5, 1000, CFGFLAG_SAVE | CFGFLAG_CLIENT | CFGFLAG_SERVER, "Network timeout")
MACRO_CONFIG_INT(ConnTimeoutProtection, conn_timeout_protection, 0, 5, 10000, CFGFLAG_SERVER, "Network timeout protection")
...
MACRO_CONFIG_INT(SvShowOthersDefault, sv_show_others_default, 1, 0, 2, CFGFLAG_SERVER, "Whether players see others by default (2 for own team)")
...
MACRO_CONFIG_STR(TrNoPermission, tr_no_permission, 512, "You don't have permission to remove capture under this name, contact moderators on discord if you think this is an error.", CFGFLAG_SERVER, "")
